	...
	if( !$('#primary').hasClass('sidebar-no') ) {
		...
		$('.content').removeClass('span9').addClass('span12');
				...
				<div class="row">
					<div class="span12">
						<ul class="filters">
							<li class="filterable-options"><a class="active" href="<?php echo  esc_url( add_query_arg( 'cat', '' ) ) ?>" data-option-value="*"><?php _e('All', 'yit'); ?></a></li>
							<?php if(!empty($cats)): ?>
								<?php foreach( $cats as $cat => $name ): ?>
									<?php if( yit_work_items_in_category($cat) ): ?>
									<li class="filterable-options"><a href="<?php echo esc_url( add_query_arg( 'cat', $cat ) ) ?>" data-option-value=".<?php echo $cat ?>"><?php echo $name ?></a></li>
									<?php endif ?>
								<?php endforeach ?>
							<?php endif ?>
						</ul>
                        ...
                        <li <?php yit_work_class( $classes ) ?>>
							...
							<div class="ch-item<?php if ( $lightbox || $details || $title ) : ?> ch-item-hover<?php endif ?>" style="background: url('<?php echo $thumbnail_url ?>') no-repeat center;">
								...
								<div class="ch-info">
									...
									<?php if( $title ): ?>
									<div class="ch-info-text<?php echo ($lightbox || $details) ? '' : ' ch-info-text-noicons' ?>">
