        ...
        if ( sizeof( $order->get_items() ) > 0 ) :
            ...
            foreach( $order->get_items() as $item) :
                ...
                if ( $_product && ! $_product->is_visible() )
                    ...
                    echo apply_filters( 'woocommerce_order_item_name', sprintf( '<a href="%s">%s</a><strong class="product-quantity">%s</strong>', get_permalink( $item['product_id'] ), $item['name'], ( ' &times; ' . $item['qty'] ) ), $item );
                ...
                if ( $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) :
                    ...
                    foreach ( $download_files as $download_id => $file ) :
                        $i++;
                        echo '<br/><small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file %s &rarr;', 'yit' ), ( count( $download_files ) > 1 ? $i : '' ) ) . '</a></small>';
                    endforeach;
