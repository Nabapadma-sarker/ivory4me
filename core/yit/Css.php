...
class YIT_Css {
    ...
    public function add_by_option( $option, $value ) {
        ...
        if( $option['id'] == 'header-height' && $value != $option['std'] ) {            
            ...
            if ( isset( $option['boxshadow'] ) && $option['boxshadow'] ) {
                $value = $option['boxshadow'] . ' ' . $value;
            }

            if ( isset( $option['opacity'] ) && isset( $value[0] ) && $value[0] == '#' ) {
        ...
        if ( preg_match_all( "/url\(\s*[\'|\"]?([A-Za-z0-9_\-\/\.\\%?&#]+)[\'|\"]?\s*\)/ix", $fileContent, $urlMatches ) )
        {
           ...
           {                                        
               ...
               {                                                                               
                   ...
                   if ( isset( $newMatchPath_parsed['port'] ) ) $newMatchPath_parsed['host'] .= ':' . $newMatchPath_parsed['port'];
